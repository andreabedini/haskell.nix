...
plan2nix args Plan {packages, extras, components, compilerVersion, compilerPackages} = do
  -- TODO: this is an awful hack and expects plan-to-nix to be
  -- called from the toplevel project directory.
  cwd <- getCurrentDirectory

  extrasNix <- fmap (mkNonRecSet . concat) . forM (Map.toList extras) $ \case
    (_name, Just (Package _v _r _flags (Just (LocalPath folder)))) -> do
    ...
    (_name, Just (Package _v _r _flags (Just (DVCS (Git url rev) subdirs)))) ->
  ...
  where
    ...
    mapKeys f = Map.fromList . fmap (Data.Bifunctor.first f) . Map.toList
      ...
      return $ \hash ->
        forM cabalFiles $ \cabalFile -> do
          let pkg = cabalFilePkgName cabalFile
              ...
              src = Just $ C2N.Git url rev (Just hash) subdir'
          createDirectoryIfMissing True (takeDirectory nixFile)
          writeDoc nixFile . prettyNix =<< cabal2nix True (argDetailLevel args) src cabalFile
          liftIO $ appendCache (argCacheFile args) url rev subdir hash pkg nix
          return $ fromString pkg $= mkPath False nix
  ...
  where
    ...
    componentNameToHaskellNixAttr pkgName n =
      case span (/= ':') n of
        ...
        _otherwise -> error ("unknown component name format " <> show n <> " for package " <> show pkgName)
