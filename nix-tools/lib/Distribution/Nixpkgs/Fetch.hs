...
data Source = Source
  { -- | URL to fetch from.
    sourceUrl :: String,
    -- | Revision to use. For protocols where this doesn't make sense (such as HTTP), this
    --   should be the empty string.
    sourceRevision :: String,
    -- | The expected hash of the source, if available.
    sourceHash :: Hash
  }
...
data DerivationSource = DerivationSource
  { -- | The kind of the source. The name of the build-support fetch derivation should be fetch<kind>.
    derivKind :: String,
    -- | URL to fetch from.
    derivUrl :: String,
    -- | Revision to use. Leave empty if the fetcher doesn't support revisions.
...
instance FromJSON DerivationSource where
  parseJSON (Object o) =
    DerivationSource (error "undefined DerivationSource.kind")
      <$> o .: "url"
      <*> o .: "rev"
      <*> o .: "sha256"
...
fetch f = runMaybeT . fetchers
  where
    fetchers :: Source -> MaybeT IO (DerivationSource, a)
    fetchers source =
      msum . (fetchLocal source :) $
        map
          (\fetcher -> fetchWith fetcher source >>= process)
          [ (False, "url", []),
